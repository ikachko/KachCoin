...
class RawTransaction:
    def __init__(self, sender_addr, recepient_addr, inputs, value, locktime):
        ...
        prCyan(Wallet.publkey_hash_from_addr(sender_addr))
        self.tx_in = dict()
    ...
    def tx_id(self, prkey):
        raw_tx = self.raw_transaction(prkey)
        txid = hashlib.sha256(hashlib.sha256(raw_tx).digest()).hexdigest()
        return txid
...
class RawCoinbaseTransaction(RawTransaction):
    def __init__(self):
...
script = (sig_script_len + tx.sig_script.hex() + pk_script_len + tx.tx_out1['pk_script'].hex())
prRed(script)
# print('--------')
# print(raw.hex())
# print('--------')
# print(raw_dict)
rawCoinBase = RawCoinbaseTransaction()
# print(rawCoinBase.ra)
